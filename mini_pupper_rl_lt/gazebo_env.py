...
class MiniPupperEnv(gym.Env):
    ...
    def make_test_cmd_beginner(self):
        #MAX_LIN_X = 0.26  # m/s
        #MAX_LIN_Y = 0.13  # m/s
    ...
    def speed_reward_comp(self, actual_v, cmd_v, max_speed):
        """
        actual_v: 実際の速度 (float)
        cmd_v: 目標速度 (float)
        max_speed: その軸の最大速度 (float)
        """
        error_v = (cmd_v - actual_v) ** 2
        speed = np.abs(cmd_v)

        # -------------------------------------------------------------
        # 鉄則：目標と現実の進行方向が「逆」なら、その時点で報酬は完全0
        # -------------------------------------------------------------
        ...
        if (cmd_v > 0.0 and actual_v < 0.0) or (cmd_v < 0.0 and actual_v > 0.0):
            ...
            # 例：最大速度の半分(0.5)で逆走したら 0.5/0.5 * 1.0 = -1.0 点
            penalty = -1.0 * (np.abs(actual_v) / max_speed)
            
            # 最低でも -1.0 でクリップ（それ以上沈まないように安全ガード）
            penalty = max(-1.0, penalty)
            
            # 停止時と同様、重み（weight_v）は最小限（0.1など）にして全体の計算を狂わせないようにする
            return penalty, 0.1
            #return 0.0, 0.0

        if speed < 0.01:  # 実質 0.0 とみなす（停止指令時）
            # -------------------------------------------------------------
            # 【停止処理】動いていたら減点（ペナルティ）
            # -------------------------------------------------------------
            ...
            base_reward_v = np.exp(-error_v / sigma_v)
